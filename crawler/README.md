...
Start with `crawler.py` — it does the crawl and drops `passwords.json`,
`urls_discovered.txt` and `crawl_log.jsonl` next to itself. The other scripts
are follow-up passes that read those back in:
...
```
crawler.py        crawl + first scan
deepscan*.py      decoder passes
cipherscan.py     cipher pass
anomaly_scan.py   structural / css tricks
*.json *.txt *.jsonl   git-ignored: results + anything that IDs the target
```
